...
from logics.customer_query_handler import process_user_message, global_last_discussed_course_name
from helper_functions.utility import check_password, log_interaction # <-- Import log_interaction
...
if prompt := st.chat_input("Ask about courses..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            response_text, course_details_df_list = process_user_message(prompt)

            # --- NEW: Log the interaction ---
            current_user = st.session_state.get("authenticated_user", "unknown_user") # Get the logged-in user
            ...
            # If course_details are returned, display them as a DataFrame
            if course_details_df_list:
                df_to_display = pd.DataFrame(course_details_df_list)

                # Reorder columns as requested earlier
                desired_column_order = [col for col in ['name', 'url', 'category', 'description', 'price', 'duration', 'skills_covered', 'entry_requirements', 'course_schedule', 'provider', 'course_code'] if col in df_to_display.columns]
                ...
                st.session_state.messages.append({"role": "assistant", "content": response_text, "data_frame": df_to_display})
            else:
